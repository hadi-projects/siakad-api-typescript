...
export class AccessLogMiddleware {
	...
	log(req: express.Request, res: express.Response, next: express.NextFunction) {
		...
		if (!req.headers.authorization) user_model.setName('auth')
	...
	private access(req: express.Request): AccessModel {
	...
	private get_action(e:string):any {
	...
	private get_resource(a:string):any {
